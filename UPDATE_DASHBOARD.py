...
def _load_cancel_exclusions():
    """Job numbers to drop from the Cancellation tab: true duplicate TGL tickets whose
    partner job was NOT canceled (a real duo ticket that resolved correctly). Maintained by
    refresh_dup_exclusions.py. Rule: exclude only if the duplicate's twin survived; if BOTH
    are canceled it still counts. Fail-safe: excludes nothing if the file is missing/bad."""
    try:
        d = json.loads((SCRIPT_DIR / "cancel_dup_exclusions.json").read_text())
        return set(str(x).strip() for x in d.get("exclude", [])), d
    except Exception:
        return set(), {}
CANCEL_EXCLUDE, CANCEL_EXCLUDE_META = _load_cancel_exclusions()
...
def parse_all(today):
    ...
    for grp, r in iter_grouped(load_rows("ROPP_Cancelations.xlsx"), "Lead Generated By", 1):
        if str(r[1]).strip() in CANCEL_EXCLUDE:   # true duplicate TGL, partner completed — don't count
            if to_date(r[9]) is not None and to_date(r[9]).year == YEAR: _CANCEL_EXCLUDED_COUNT += 1
            continue
